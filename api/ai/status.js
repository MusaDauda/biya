...
function resolveOne(providerName, modelOverride, explicitKey, explicitBaseUrl) {
  if (!providerName) return null;
  const name = providerName.trim().toLowerCase();
  const preset = PRESETS[name];
  const baseURL = explicitBaseUrl || preset?.baseURL;
  if (!baseURL) return null;
  const apiKey = explicitKey || (preset ? process.env[preset.keyEnv] : void 0) || process.env[`${name.toUpperCase()}_API_KEY`] || (preset?.keyOptional ? "local" : "");
...
function resolveProviders() {
  const primary = resolveOne(
    process.env.LLM_PROVIDER ?? (process.env.GROQ_API_KEY ? "groq" : void 0),
  ...
  return [primary, fallback].filter((p) => p !== null);
}
...
function toolCallsFromText(text, knownNames) {
  if (!text) return [];
  const fenced = text.match(/```(?:json)?\s*([\s\S]*?)```/);
...
function isToolGenerationFailure(err) {
  if (err?.status !== 400) return false;
  const text = String(err?.message ?? "");
  return /failed to call a function|tool_use_failed|failed_generation|tool call validation failed|was not in request\.tools/i.test(text);
}
function rateLimitDelayMs(err) {
  if (err?.status !== 429) return null;
  const header = err?.headers?.["retry-after"];
  if (header && !Number.isNaN(Number(header))) return Math.ceil(Number(header) * 1e3);
  const stated = String(err?.message ?? "").match(/try again in\s+(?:(\d+)\s*m)?\s*([\d.]+)\s*s/i);
  if (stated) {
    const minutes = Number(stated[1] ?? 0);
    const seconds = Number(stated[2] ?? 0);
    return Math.ceil((minutes * 60 + seconds) * 1e3) + 250;
  }
  return 2e3;
}
var sleep2 = (ms) => new Promise((r) => setTimeout(r, ms));
var TOOL_RETRIES = 3;
var MAX_RATE_LIMIT_WAIT_MS = 8e3;
var OpenAICompatibleClient = class {
  ...
  async chat(request) {
    ...
    let toolCalls = (choice?.message?.tool_calls ?? []).filter((c) => c.type === "function" || c.function).map((c) => ({
  ...
  async chat(request) {
    ...
    for (const client of this.clients) {
      try {
        ...
        last = err;
        console.warn(
          `[llm] ${client.id} failed, trying next:`,
...
var TOOLS = [
  ...
  },
  {
    ...
    handler: (ctx, a) => rpc(ctx, "ai_list_transactions", {
      p_user: ctx.userId,
      p_days: a.days ?? 7,
      p_limit: a.limit ?? 20
    })
  },
  {
    ...
    tier: "read",
    description: "Totals by counterparty and by day, with each counterparty's category. Use for 'how much did I spend on food this week' instead of adding transactions up yourself.",
    parameters: {
      type: "object",
      properties: { days: { type: "integer", description: "Default 7, max 365." } },
      required: []
    },
    handler: (ctx, a) => rpc(ctx, "ai_spending_summary", { p_user: ctx.userId, p_days: a.days ?? 7 })
  },
  {
    name: "get_goals",
    tier: "read",
  ...
  },
  {
    name: "find_payee",
    tier: "read",
    description: "Find who to pay, by name or 10-digit Biya code. Returns candidates with user ids. Call this before propose_payment. If more than one matches, ask which; never guess.",
    parameters: {
      type: "object",
  ...
  },
  {
    ...
    handler: (ctx, a) => rpc(ctx, "ai_business_summary", { p_user: ctx.userId, p_days: a.days ?? 7 })
  },
  // -------------------------------------------------------------------------
  ...
  // -------------------------------------------------------------------------
  {
    ...
    tier: "propose",
    interactive: true,
    description: "Propose a payment. Does NOT move money; the user must confirm with their PIN. Amount in NGN kobo: \u20A61,200 is 120000. Get payee_user_id from find_payee first.",
    parameters: {
      type: "object",
      properties: {
        payee_user_id: {
    ...
    handler: (ctx, a) => rpc(ctx, "ai_propose_payment", {
